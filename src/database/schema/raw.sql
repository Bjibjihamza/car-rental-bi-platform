-- ============================================
-- Système de Gestion de Location de Voitures avec IoT (Oracle 12c+)
-- ============================================

-- Optionnel : basculer sur un schéma (si déjà créé)
-- ALTER SESSION SET CURRENT_SCHEMA = CARR_RENTAL;

-- ============================================
-- Helpers : trigger générique pour updated_at
-- ============================================
...
END;
/
-- On utilisera un petit trigger par table (plus clair) plutôt que d’appeler la proc,
-- car Oracle ne permet pas d’assigner directement depuis une proc dans le NEW.*

-- ============================================
-- Table des Branches/Agences
-- ============================================
CREATE TABLE Branches (
    ...
    phone        VARCHAR2(20),
    email        VARCHAR2(100),
    manager_id   NUMBER(19), -- FK après Managers
...
-- Ajout FK manager_id -> Managers (évitons la dépendance circulaire à la création initiale)
...
CREATE TABLE IoT_Devices (
    ...
    device_type      VARCHAR2(50) NOT NULL, -- GPS, Fuel Monitor, etc.
...
CREATE TABLE Cars (
    ...
    daily_rate        NUMBER(10,2), -- optionnel si tarif par voiture
    created_at        TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
    updated_at        TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
    CONSTRAINT uq_cars_plate UNIQUE (license_plate),
    CONSTRAINT ck_car_year CHECK (year BETWEEN 1980 AND EXTRACT(YEAR FROM SYSTIMESTAMP) + 1),
...
-- ============================================
-- Table des Locations (contrats actifs)
-- ============================================
...
-- Empêcher le chevauchement des locations pour la même voiture (INSERT/UPDATE)
...
-- ============================================
-- Table des Données IoT (métier simplifiée)
-- ============================================
CREATE TABLE IoT_Data (
    ...
    speed            NUMBER(5,2),   -- km/h
    fuel_level       NUMBER(5,2),   -- %
    engine_status    VARCHAR2(3),   -- 'On'/'Off'
    battery_voltage  NUMBER(4,2),
    temperature      NUMBER(5,2),   -- °C moteur
...
-- ============================================
-- Table de Télémétrie détaillée (raw)
-- ============================================
CREATE TABLE car_telemetry (
    ...
    ignition_on         CHAR(1) DEFAULT 'N' NOT NULL, -- Y/N
    ...
    message_uid         RAW(16), -- UUID stocké en 16 octets si souhaité
...
CREATE TABLE IoT_Alerts (
    ...
    resolved        CHAR(1) DEFAULT 'N' NOT NULL, -- Y/N
...
-- ============================================
-- Table des Trajets/Itinéraires (agrégées)
-- ============================================
CREATE TABLE Trips (
...
-- Voitures disponibles avec dispositifs IoT
...
-- Locations actives avec derniers points IoT
...
SELECT 
    ...
    (SELECT d.latitude
      ...
      FETCH FIRST 1 ROWS ONLY) AS last_latitude,
    (SELECT d.longitude
       FROM IoT_Data d
      WHERE d.car_id = c.car_id
      ORDER BY d.ts_utc DESC
      FETCH FIRST 1 ROWS ONLY) AS last_longitude,
    (SELECT d.fuel_level
       FROM IoT_Data d
      WHERE d.car_id = c.car_id
      ORDER BY d.ts_utc DESC
      FETCH FIRST 1 ROWS ONLY) AS current_fuel_level
FROM Rentals r
