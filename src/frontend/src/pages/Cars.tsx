...
type CategoryRow = { CATEGORY_ID: number; CATEGORY_NAME: string };
...
function badgeTone(status: string) {
  const map: Record<
    string,
    "green" | "blue" | "amber" | "gray" | "purple" | "red" | "indigo"
  > = {
    AVAILABLE: "green",
    RENTED: "blue",
    MAINTENANCE: "amber",
  ...
  return map[String(status || "").toUpperCase()] || "red";
}

/* ================= SUB-COMPONENT: CAR DRAWER ================= */
function CarDetailsDrawer({
  car,
  onClose,
}: {
  car: CarRow | null;
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="relative rounded-xl overflow-hidden border border-white/10 shadow-lg bg-neutral-900">
            {car.IMAGE_URL ? (
              ...
              <div className="h-56 grid place-items-center text-neutral-500">
                <CarIcon size={48} />
              </div>
            )}
            <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-4">
              ...
              <div className="text-neutral-300 text-sm">
                {car.MODEL_YEAR} • {categoryLabel(car.CATEGORY_ID)}
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <div className="flex-1 rounded-xl bg-white/5 p-3 border border-white/5 text-center">
              ...
              <div className="mt-1 flex justify-center">
                <Badge tone={badgeTone(car.STATUS)}>{car.STATUS}</Badge>
              </div>
            </div>
            <div className="flex-1 rounded-xl bg-white/5 p-3 border border-white/5 text-center">
              <div className="text-xs text-neutral-500 uppercase tracking-wide">
              ...
              <div className="mt-1 font-mono text-white font-bold">
                {car.LICENSE_PLATE}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl bg-indigo-500/10 p-5 border border-indigo-500/20 relative overflow-hidden">
            <div className="relative z-10">
              <h4 className="text-sm font-bold text-indigo-300 mb-1 flex items-center gap-2">
                <Gauge size={14} /> IoT Telemetry
              </h4>
              <p className="text-xs text-indigo-200/60 mb-3">
                Device ID: {car.DEVICE_ID ? `#${car.DEVICE_ID}` : "Not Linked"}
              </p>
...
function CreateCarModal({
  ...
  // keep BRANCH_ID synced for manager (locked)
  useEffect(() => {
    if (!isOpen) return;
    if (!isSup && user?.branchId) {
      setForm((prev) => ({
        ...prev,
  ...
  }, [isOpen, isSup, user?.branchId]);

  async function fetchDevicesByBranch(branchId: string | number | null) {
    if (!branchId) {
      setDevices([]);
      return;
    }
    try {
      const res = await fetch(
        `${API_URL}/api/v1/devices/available?branchId=${branchId}`,
        {
          headers: {
  ...
  // On open: load branches + categories, then load devices for selected branch
  useEffect(() => {
    if (!isOpen) return;
    ...
    Promise.all([
      ...
      .then(([bData, cData]) => {
        ...
        const branchId = form.BRANCH_ID || (user?.branchId ? String(user.branchId) : "");
        if (branchId) fetchDevicesByBranch(branchId);
      })
      .catch(console.error);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, token]);

  // When BRANCH changes: reset device + refetch devices for that branch
  useEffect(() => {
    if (!isOpen) return;
    ...
    // reset selection (prevents Rabat+Casablanca mix)
    setForm((prev) => ({ ...prev, DEVICE_ID: "" }));
    fetchDevicesByBranch(form.BRANCH_ID);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const InputClass =
    "w-full rounded-xl bg-neutral-900 border border-white/10 px-3 py-2.5 text-white text-sm focus:border-indigo-500 outline-none transition";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
      <div className="w-full max-w-3xl overflow-hidden rounded-2xl border border-white/10 bg-[#121212] shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 max-h-[75vh] overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="col-span-1 md:col-span-2 text-xs font-bold text-indigo-400 uppercase tracking-wider mb-[-10px]">
            Assignments
          </div>
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              disabled={!isSup} // manager can't change branch
            >
            ...
            {isAddingCat ? (
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={catLoading}
                  className="px-3 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500 font-bold text-xs"
                >
                  {catLoading ? "..." : "SAVE"}
                </button>
                <button
                  onClick={() => setIsAddingCat(false)}
                  className="px-3 rounded-lg bg-neutral-700 text-white hover:bg-neutral-600 text-xs"
                >
                  X
                </button>
              </div>
          ...
          <div className="col-span-1 md:col-span-2 text-xs font-bold text-indigo-400 uppercase tracking-wider mt-2 mb-[-10px]">
            Details
          </div>
          ...
          <div className="space-y-1">
            ...
            {form.BRANCH_ID && devices.length === 0 && (
              <div className="text-[11px] text-amber-400">
                No available devices for this branch.
              </div>
            )}
          </div>
        ...
        <div className="border-t border-white/10 p-6 bg-[#18181b] flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 rounded-xl border border-white/10 py-3 text-sm font-bold text-white hover:bg-white/5 transition"
          >
            Cancel
          </button>
          <button
            ...
            disabled={loading}
            className="flex-1 rounded-xl bg-indigo-600 py-3 text-sm font-bold text-white hover:bg-indigo-500 disabled:opacity-50 transition shadow-lg shadow-indigo-500/20"
          >
...
export function CarsPage() {
  ...
  const mergedCars = useMemo(() => {
    return cars.map((c) => {
      if (activeCarIds.has(c.CAR_ID)) return { ...c, STATUS: "DRIVING" };
      return c;
    });
  }, [cars, activeCarIds]);
  ...
  const filtered = useMemo(() => {
    return mergedCars.filter((c) => {
      const matchesQ = [c.MAKE, c.MODEL, c.VIN, c.LICENSE_PLATE]
        .join(" ")
        .toLowerCase()
  ...
  const HeaderControls = (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <select
        className="h-9 rounded-lg border border-white/10 bg-white/5 px-3 text-sm text-white bg-[#18181b]"
        value={category}
      ...
      <div className="flex h-9 items-center rounded-lg border border-white/10 bg-white/5 p-1">
        <button
          ...
          className={`px-2 rounded ${
            viewMode === "cards" ? "bg-indigo-600 text-white" : "text-neutral-400"
          }`}
        >
          <LayoutGrid size={16} />
        </button>
        <button
          ...
          className={`px-2 rounded ${
            viewMode === "table" ? "bg-indigo-600 text-white" : "text-neutral-400"
          }`}
        >
          <List size={16} />
        </button>
      ...
      <button
        onClick={fetchCars}
        className="h-9 w-9 grid place-items-center rounded-lg border border-white/10 bg-white/5 text-neutral-400 hover:text-white"
      >
        <RefreshCw size={16} />
      </button>

      {/* If you want managers to add cars too, change condition to: (isSup || user?.role === "manager") */}
      {isSup && (
        <button
          onClick={() => setCreateOpen(true)}
          className="flex h-9 items-center gap-2 rounded-lg bg-emerald-600 px-4 text-sm font-bold text-white hover:bg-emerald-500"
        >
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <Card
        title={isSup ? "Global Fleet" : "Branch Fleet"}
      ...
      >
        {loading ? (
          ...
          <div className="py-20 text-center text-neutral-500">
            <Search className="mx-auto mb-3" /> No vehicles found.
          </div>
        ) : viewMode === "table" ? (
          <DataTable
            rows={filtered}
            cols={[
              ...
              },
              {
                ...
                header: "Plate",
                render: (r) => (
                  <code className="bg-white/10 px-1 rounded">{r.LICENSE_PLATE}</code>
                ),
              },
              {
                key: "STATUS",
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {filtered.map((c) => (
              <div
                key={c.CAR_ID}
                className="group relative overflow-hidden rounded-2xl border border-white/5 bg-[#18181b] hover:border-indigo-500/30 transition-all"
              >
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex gap-2 text-xs">
                    <div className="bg-white/5 p-2 rounded flex-1 flex items-center gap-2 text-neutral-400">
                      <Gauge size={12} /> {fmtKm(c.ODOMETER_KM)}
                    </div>
                    <div className="bg-white/5 p-2 rounded flex-1 flex items-center gap-2 text-neutral-400">
                      <MapPin size={12} /> {c.BRANCH_CITY || "N/A"}
                    </div>
                  </div>
                  <button
                    onClick={() => setSelectedCar(c)}
                    className="mt-4 w-full rounded-xl bg-white/5 py-2 text-sm font-medium text-neutral-300 hover:bg-white/10"
                  >
                    View Details
                  </button>
                </div>
