...
// (Managers can Register Device ONLY for their branch; Supervisor can pick any branch)
...
function NetworkStat({ label, value, icon: Icon, color }: any) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/5 bg-[#121212]/60 p-5 shadow-xl">
      <div className={`absolute -right-4 -top-4 h-20 w-20 rounded-full ${color} opacity-10 blur-xl`} />
...
export function DevicesPage() {
  ...
  const canCreate = !!user; // both can create (branch restrictions applied)
  ...
  async function saveDevice() {
    ...
    try {
      const payload = {
        ...
        STATUS: form.STATUS,
        // supervisor chooses branch, manager ignored by backend (forced to his branch)
        BRANCH_ID: isSup ? (form.BRANCH_ID ? Number(form.BRANCH_ID) : null) : null,
      };

      const url = editMode ? `${API_URL}/api/v1/devices/${editMode.DEVICE_ID}` : `${API_URL}/api/v1/devices`;
  ...
  function openCreate() {
    ...
    // ✅ manager branch auto-filled (display only)
    const defaultBranch =
      !isSup && user?.branchId ? String(user.branchId) : "";
  ...
  const stats = useMemo(
    () => ({
      ...
      active: devices.filter((d) => String(d.STATUS).toUpperCase() === "ACTIVE").length,
      inactive: devices.filter((d) => String(d.STATUS).toUpperCase() === "INACTIVE").length,
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <DataTable
          ...
          cols={[
                    ...
                    {r.BRANCH_NAME ? (
                      <>
                        ...
                        <span className="text-[10px] text-neutral-500">
                          {r.BRANCH_NAME} {r.IS_INSTALLED ? "(Vehicle)" : "(Stock)"}
                        </span>
                      </>
                    ) : (
            ...
            },
            {
              ...
              render: (r) => (
                <div className="flex items-center gap-2 text-xs">
                  <Signal size={12} className={String(r.STATUS).toUpperCase() === "ACTIVE" ? "text-emerald-500" : "text-neutral-600"} />
            ...
            },
            {
              ...
              render: (r) => {
                const canEditThis =
                  isSup || (user?.branchId != null && r.ACTUAL_BRANCH_ID === user.branchId);
                ...
                return (
                  <div className="flex items-center gap-2 justify-end">
                    ...
                    {isSup && (
                      <button
                        onClick={() => deleteDevice(r.DEVICE_ID)}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="space-y-4 rounded-xl border border-white/5 bg-white/[0.02] p-4">
                  ...
                  <div className="grid grid-cols-2 gap-y-4 text-sm">
                    <div>
                      <div className="text-neutral-500 text-xs">IMEI</div>
                      <div className="text-white font-mono">{selected.DEVICE_IMEI || "—"}</div>
                    </div>
                    <div>
                      <div className="text-neutral-500 text-xs">Firmware</div>
                      <div className="text-white font-mono">{selected.FIRMWARE_VERSION || "—"}</div>
                    </div>
                    <div>
                      <div className="text-neutral-500 text-xs">Registered</div>
                      <div className="text-white">{fmtDate(selected.CREATED_AT)}</div>
                    </div>
                    <div>
                      <div className="text-neutral-500 text-xs">Last Ping</div>
                      <div className="text-white">{fmtDate(selected.LAST_SEEN_AT)}</div>
                    </div>
                ...
                {selected.CAR_ID ? (
                  <div className="rounded-xl bg-emerald-500/10 p-4 border border-emerald-500/20">
                    <h4 className="text-sm font-bold text-emerald-300 mb-1">Active Assignment</h4>
                  ...
                  <div className="rounded-xl bg-amber-500/10 p-4 border border-amber-500/20">
                    <h4 className="text-sm font-bold text-amber-300 mb-1">Unassigned</h4>
      ...
      {/* CREATE / EDIT MODAL (Manager + Supervisor) */}
      {modalOpen && user && (
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    className="w-full rounded-xl bg-neutral-900 border border-white/10 px-3 py-2.5 text-white text-sm focus:border-indigo-500 outline-none transition font-mono"
              ...
              {isSup ? (
                ...
                <div className="space-y-1">
                  ...
                  <input
                    disabled
                    className="w-full rounded-xl bg-neutral-900/60 border border-white/10 px-3 py-2.5 text-white/70 text-sm outline-none"
                    value={
                      user?.branchId
                        ? `Branch #${user.branchId} (your branch)`
                        : "No branch assigned"
                    }
                  />
                  <div className="text-[11px] text-neutral-500">
                    Managers can only register devices for their own branch.
                  </div>
                </div>
              )}
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button
                  onClick={saveDevice}
                  disabled={saving || !form.DEVICE_CODE || (!isSup && !user?.branchId)}
